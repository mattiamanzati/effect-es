...
const ifNotReceivedBefore = AtMostOnce.make(DecreaseStockOnShipmentType, (event) => event.id)
...
export const registerSaga = Sharding.registerEntity(DecreaseStockOnShipmentType, (sagaId, dequeue) =>
  pipe(
    PoisonPill.takeOrInterrupt(dequeue),
    Effect.flatMap(
      ifNotReceivedBefore(sagaId)((event) =>
        Effect.gen(function*(_) {
          if (event.body._tag === "ProductShipped") {
            ...
            const message = yield* _(
              Envelope.makeRelated({ _tag: "Decrease", amount: event.body.amount })
            )
            ...
            const newStock = yield* _(
              inventoryMessenger.send(event.body.productId)(
                Inventory.GetCurrentStock(yield* _(Envelope.makeRelated({ _tag: "GetCurrentStock" })))
              )
            )
            yield* _(Effect.log("Stock of product " + event.body.productId + " is now " + newStock))
          }
        }).pipe(Envelope.withOriginatingEnvelope(event))
      )
    ),
